...
describe("content", () => {
  ...
  it("handles headings", () => {
    ...
    // errors
    assertThrows(() => content("£ title £", "txt"), Error, "bad heading tags");
    assertThrows(
      () => content("£7 title £7", "txt"),
      Error,
      "bad heading tags"
    );
    assertThrows(
      () => content("£1 title £2", "txt"),
      Error,
      "mismatched heading tags"
    );
  });
  ...
  it("handles strong text", () => {
    ...
    // errors
    assertThrows(() => content("*test", "txt"), Error, "unterminated '*' tag");
  });
  ...
  it("handles emphasised text", () => {
    ...
    // errors
    assertThrows(() => content("_test", "txt"), Error, "unterminated '_' tag");
  });
  ...
  it("parses ligatures", () => {
    ...
    // errors
    assertThrows(() => content("{as}", "txt"), Error, "unrecognised ligature");
  });
  ...
  it("handles footnote references", () => {
    ...
    // errors
    assertThrows(() => content("[n1", "txt"), Error, "unterminated '[' tag");
  });
